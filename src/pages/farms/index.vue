    ...
    async updateLabelizedAmms()
    {
      ...
      try{
        this.poolsDatas = await fetch(
          DEVNET_MODE ? 'https://api.croppppp.com/' : 'https://api.finance.fyfy/pools/'
        ).then(res => res.json());
      }
      catch{
        this.poolsDatas = []
    ...
    async updateFarms() {
      ...
      for (const [poolId, farmInfo] of Object.entries(this.farm.infos)) {
        ...
        if((newFarmInfo as any).poolInfo.is_allowed > 0 || 
          ...
          (newFarmInfo as any).twitterShare = `http://twitter.com/share?text=Earn ${(newFarmInfo as any).reward.name} with our new farm on @FyfyFinance&url=https://finance.fyfy/farms/?s=${(newFarmInfo as any).poolId} &hashtags=${(newFarmInfo as any).lp.coin.symbol},${(newFarmInfo as any).lp.pc.symbol},yieldfarming,Solana`
