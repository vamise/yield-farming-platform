          ...
          if(element.pfo == true){
            ...
            try{
              responseData2 = await fetch(
                'https://api.fyfy.app/pfo/?farmId='+ this.labelizedAmms[element.slug].pfarmID + '&t='+ Math.round(moment().unix()/60)
              ).then(res => res.json());
            }
            catch{
            }
...
.fertilizer .list{
  ...
  .singleFarm{
    ...
    .labelaner{
        background:#2c73de;
        background:linear-gradient(180deg, #2c73de 0%, #14bb89 100%);
        color:#fff;
        padding:10px 0;
