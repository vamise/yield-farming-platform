<template>
  <div>
    ...
    <div  class="fertilizer uniq cont" v-if="initialized">
       ...
       <div class="card-body" style="grid-row-gap: 0; row-gap: 0; padding-bottom: 15px">
          ...
          <Row class="full-border pf-margin-top pf-padding-top"  :span="isMobile ? 24 : 12">
                    ...
                    <div class="share text-center">
                        <div class="btncontainer">
                          <Button
                            ...
                            class="button_div"
                            style="z-index: 999; width: 100%"
                            @click="startFollow('https://api.fyfy.app/pfo/follow/?spl='+ $accessor.wallet.address +'&farmId='+ farm.pfarmID)"
                          >
                             + Follow
                          </Button>
                        </div>
            ...
            <Col :span="isMobile ? 24 : 12" :class="isMobile ? ' steps' : 'steps'">
                ...
                <div class="done" >
                    <span class="span first"><img src="https://finance.fyfy/distant/check-one.png?t=1" alt=""></span>
                ...
                <div :class="farm.pla_ts < currentTimestamp ? 'done' : 'notdone' " >
                    ...
                    <span v-else class="span"><img src="https://finance.fyfy/distant/check-one.png?t=1" alt=""></span>
...
//    RadioButton
  ...
  methods: {
        ...
        responseData.forEach(async (element:any) => {
          ...
          if(element.pfo == true){
            ...
            if(query.get('f') && element.slug == query.get('f')){
              element.calculateNextStep = 'Test'
              this.ammId = element.ammID
              this.labelizedAmms[element.ammID] = element;
              try{
                responseData2 = await fetch(
                  'https://api.fyfy.app/pfo/?farmId='+ this.labelizedAmms[element.ammID].pfarmID + '&t='+ Math.round(moment().unix()/60)
                ).then(res => res.json());
              }
              catch{
              }
                ...
                //this.labelizedAmms[element.ammID].twitterShare = `http://twitter.com/share?text=Earn ${this.labelizedAmms[element.ammID].tokenA.symbol} with our new farm on @FyfyFinance&url=https://finance.fyfy.app?s=${newFarmInfo.poolId} &hashtags=${this.labelizedAmms[element.ammID].tokenA.symbol},${this.labelizedAmms[element.ammID].tokenB.symbol},yieldfarming,Solana`

                    document.title = 'Fertilizer - FyfyFinance x ' + element.name ;
    ...
    async updateFarms() {
                ...
                if(this.labelizedAmms[newFarmInfo.poolId].pfo == true && newFarmInfo.poolId == this.labelizedAmms[newFarmInfo.poolId].pfarmID){       
                  ...
                  if(query.get('f') && this.labelizedAmms[newFarmInfo.poolId].slug == query.get('f')){
                    isPFO = true;
                     ...
                     newFarmInfo.twitterShare = `https://twitter.com/share?text=Earn ${newFarmInfo.reward.name} with our new farm on @FyfyFinance&url=https://finance.fyfy.app?s=${newFarmInfo.poolId} &hashtags=${newFarmInfo.lp.coin.symbol},${newFarmInfo.lp.pc.symbol},yieldfarming,Solana`
                    ...
                    document.title = 'Fertilizer - FyfyFinance x ' + this.labelizedAmms[newFarmInfo.poolId].name ;

                    let responseData
                    try{
                      responseData = await fetch(
                        'https://api.fyfy.app/pfo/?farmId='+ this.labelizedAmms[newFarmInfo.poolId].pfarmID + '&t='+ Math.round(moment().unix()/60)
                      ).then(res => res.json());
                    }
                    catch{
                    }
                      ...
                      if(responseData[this.wallet.address]){
                        if(responseData[this.wallet.address].submit > 0){
                          ...
                          this.shareWalletAddress = "https://finance.fyfy.app/fertilizer/project/?f=" + this.labelizedAmms[newFarmInfo.poolId].slug + "&r=" + this.wallet.address;
      ...
      try{
        responseData = await fetch(
          'https://api.fyfy.app/pfo/?farmId='+ this.labelizedAmms[this.ammId].pfarmID + '&t='+ Math.round(moment().unix()/60)
        ).then(res => res.json());
      }
      catch{
      }
        ...
        if(responseData[this.wallet.address]){
          if(responseData[this.wallet.address].submit > 0){
            ...
            this.shareWalletAddress = "https://finance.fyfy/fertilizer/project/?f=" + this.labelizedAmms[this.ammId].slug + "&r=" + this.wallet.address;
            let shareWalletAddressEsc = "https://finance.fyfy/fertilizer/project/?f=" + this.labelizedAmms[this.ammId].slug + "%26r=" + this.wallet.address;
